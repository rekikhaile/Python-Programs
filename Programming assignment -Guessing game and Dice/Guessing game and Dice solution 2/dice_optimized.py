...
def test(dice,sides,rolls):
    for i in range(rolls):
        yield roll(dice,sides)


# Instead of generating a new sequence 5 times, we create one
# at the beginning of the function and then pick a random element
# from it.
# This reduces the overhead of creating ranges with random.randint() or random.randrange().
def roll(dice,sides):
    sides_seq = [i+1 for i in range(sides)]
    return sum(sides_seq[int(random.random() * len(sides_seq))] for i in range(dice))
